...
下面展示了一个监听本进程在 CPU 0 上 3 秒的性能计数器命令。
...
由 command_script.py 完成，该脚本是 report 命令的包装脚本。
...
由 recv_binary_cache.py 完成，该工具主要根据 perf.data 里面记录的相关文件和库，以及它们的 buildid 在用户指定的路径中去寻找对应的 ELF 作为符号表文件用于回栈或者打印函数名等。
