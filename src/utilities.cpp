...
bool IsContainerProcess(pid_t pid)
{
    ...
    std::lock_guard<std::mutex> lock(cacheMutex);
    cache[pid] = isContainer;
    return isContainer;
}

#if defined(is_ohos) && is_ohos
namespace {
class GlobalNamespaceFd {
public:
    static int GetOriginalNsFd()
    {
        std::call_once(initFlag_, []() {
            const std::string originalNsPath = "/proc/self/ns/mnt";
            originalNsFd_ = open(originalNsPath.c_str(), O_RDONLY | O_CLOEXEC);
            if (originalNsFd_ < 0) {
                HLOGE("Failed to open original namespace %s: %s", originalNsPath.c_str(), strerror(errno));
            } else {
                HLOGD("Global original namespace fd initialized: %d", originalNsFd_);
            }
        });
        return originalNsFd_;
    }
    ...
    static void Cleanup()
    {
        if (originalNsFd_ >= 0) {
            close(originalNsFd_);
            originalNsFd_ = -1;
            HLOGD("Global original namespace fd closed");
        }
    }

private:
    static int originalNsFd_;
    static std::once_flag initFlag_;
};

int GlobalNamespaceFd::originalNsFd_ = -1;
std::once_flag GlobalNamespaceFd::initFlag_;

struct GlobalNamespaceFdCleanup {
    ~GlobalNamespaceFdCleanup()
    {
        GlobalNamespaceFd::Cleanup();
    }
};

static GlobalNamespaceFdCleanup cleanup_;
}
#endif

NamespaceSwitcher::NamespaceSwitcher(pid_t targetPid)
    : targetPid_(targetPid)
{
#if defined(is_ohos) && is_ohos
    originalNsFd_ = GlobalNamespaceFd::GetOriginalNsFd();
    if (originalNsFd_ < 0) {
        HLOGE("Global original namespace fd is invalid");
        return;
    }

    const std::string targetNsPath = StringPrintf("/proc/%d/ns/mnt", targetPid_);
    targetNsFd_ = open(targetNsPath.c_str(), O_RDONLY | O_CLOEXEC);
    if (targetNsFd_ < 0) {
        HLOGE("Failed to open target namespace %s: %s", targetNsPath.c_str(), strerror(errno));
        return;
    }
    HLOGD("NamespaceSwitcher created for pid %d", targetPid_);
#endif
}
...
bool NamespaceSwitcher::SwitchToTarget()
{
#if defined(is_ohos) && is_ohos
    if (!IsValid()) {
        HLOGE("NamespaceSwitcher is not valid");
        return false;
    }

    if (setns(targetNsFd_, CLONE_NEWNS) < 0) {
        HLOGE("Failed to switch to target namespace for pid %d: %s", targetPid_, strerror(errno));
        return false;
    }

    switched_ = true;
    HLOGD("Switched to namespace of pid %d", targetPid_);
    return true;
#else
    return false;
#endif
...
bool NamespaceSwitcher::RestoreOriginal()
{
#if defined(is_ohos) && is_ohos
    if (!switched_ || originalNsFd_ < 0) {
        return false;
    }

    if (setns(originalNsFd_, CLONE_NEWNS) < 0) {
        HLOGE("Failed to restore original namespace: %s", strerror(errno));
        return false;
    }

    switched_ = false;
    HLOGD("Restored to original namespace");
    return true;
#else
    return false;
#endif
...
bool NamespaceSwitcher::IsValid() const
{
    return originalNsFd_ >= 0 && targetNsFd_ >= 0;
}
